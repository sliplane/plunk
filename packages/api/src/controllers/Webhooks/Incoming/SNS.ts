...
export class SNSWebhook {
  ...
  public async receiveSNSWebhook(req: Request, res: Response) {
    try {
      ...
      signale.info(
        `Transactional email event received: ${body.eventType} for ${email.contact.email} from ${project.name}; message id: ${body.mail.messageId}`
      );
      ...
      if (body.eventType === "Complaint" || body.eventType === "Bounce") {
        signale.warn(
          `${
            body.eventType === "Complaint" ? "Complaint" : "Bounce"
          } received for ${email.contact.email} from ${project.name}`
        );
